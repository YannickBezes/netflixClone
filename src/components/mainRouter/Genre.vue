  ...
  <div>
    <div class="main" v-if="true">
      ...
      <div class="card-frame">
        <content-card class="card" :key="item.id" v-for="item in list" :item="item"></content-card>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    if (this.result.length === 0) {
      this.result = this.database.filter(item => item.director.indexOf(this.$route.query.p) >= 0);
    }
