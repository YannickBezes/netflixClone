<template>
  <div class="container">
        ...
        <div class="mainVideo-infos">
          ...
          <div class="buttons">
            ...
            <router-link tag="div" :to="{ query: { id: database[randomNumber].id } }">
              <div class="moreInformation" @click="moreInformation">
    ...
    <div class="mobile">
      <img :src="database[randomNumber].logo" alt=""/>
      <div class="description">
        <p>Witness the birth of Mexico's drug war in the 1980s in this bold new story of Narcos that tells the true story of the rise of the Guadalajara Cartel.</p>
      </div>

      <div rel="preload" class="buttons">
        <router-link
          :to="{ path: '/watch', query: { id: database[randomNumber].id } }"
          tag="div"
          class="play"
...
export default {
  ...
  mounted() {
    ...
    setTimeout(() => {
      ...
      document.querySelector('.picturing').children[0].style.marginTop = '8vw';
      document.querySelector('.picturing').children[0].style.width = '25%';
    }, 10000);
  },
  created() {
     if (this.selectType === 'Film' || this.selectType === 'Series') {
      this.database = this.database.filter(item => item.type === this.selectType);
    }

    this.randomNumber = Math.floor(Math.random() * this.database.length);
  ...
  watch: {
    '$store.state.bigVideoPlayerControl'() {
      if (this.$store.state.bigVideoPlayerControl === false) {
        this.$refs['bigVideo'].pause();
      } else {
        this.$refs['bigVideo'].play().catch(() => {});
      }
    },
    ...
    $route(route) {
      if (
        ...
        route.fullPath === '/series' ||
        route.fullPath === '/movies'
      ) {
        this.$refs['bigVideo'].play().catch(() => {});
