<template>
  <div>
    <div id="default" class="card">
      <div class="number" :style="numberStyle">{{ item.toplist }}</div>
      <div class="card-default" :style="cardDefaultStyle" @mouseenter="mouseOverDefault"
            ...
            <transition name="fade" mode="out-in" v-if="infoTransition" appear>
              <div class="info" :style="infoStyleControl">
                <div class="info-top">
                  <div class="info-top-left">
                    ...
                    >
                      <i
                        :class="
                          isAddedToList ? 'bi bi-check' : 'bi bi-plus'
                        "
                      ></i>
                    </div>
                    <div rel="preload" @click="likeButton" class="buttons">
                      <i
                        :class="
                          isLiked
                    ...
                    <div rel="preload" @click="dislikeButton" class="buttons">
                      <i
                        :class="
                          isDisliked
                ...
                <div class="info-bottom" style="margin-left: 0.65vw">
                  <p :key="index" v-for="(item, index) in item.subcategories">
                    {{ item }}
                    <label v-if="index < subCategoriesLength">●</label>
                  </p>
                </div>
              </div>
            </transition>
    ...
    <div id="mobile" class="card">
      <router-link tag="div" @click.native="mobileOpenPopup" class="card-mobile" :to="{ query: { id: item.id } }">
        <div class="number" style="font-size:19vw;z-index:2" :style="numberStyle">{{ item.toplist }}</div>
        <img style="border-radius:0.7vw ; width:13vw;margin-left:8vw;z-index:3" :src="item.imgv" alt=""/>
      </router-link>
    </div>
  </div>
</template>
...
<script>
import BoxVideo from './BoxVideoTop.vue';

export default {
  components: {
    BoxVideo
  },
  props: [ 'item', 'lengthNumber', 'title', 'getIndex' ],
  data() {
    return {
  ...
  created() {
    if (this.getIndex === 0) {
      this.numberStyle = 'margin-left:1vw';
    } else if (this.getIndex === 9) {
      this.numberStyle = 'font-size: 11.5vw;margin-top:-1vw;margin-left:-0.9vw';
    }
    ...
    if (this.lengthNumber === 0) {
      this.marginValue = 4;
    } else if (this.lengthNumber === 5) {
      this.marginValue = -3.8;
    } else if (this.lengthNumber > 3 && this.lengthNumber % 6 === 0) {
      this.marginValue = 4;
    }

    for (let i = 11; i < 40; i += 6) {
      if (this.lengthNumber === i) {
        this.marginValue = -3.8;
      }
    }
  },
    ...
    mouseOverDefault() {
      ...
      this.overInterval = setInterval(() => {
        ...
        if (counter === 1) {
          ...
          setTimeout(() => {
            let cardCounter = 0;
            this.$store.state.cardInterval = setInterval(() => {
              cardCounter++;
...
.number {
  ...
  font-size: 15vw;
  color: black;
  -webkit-text-stroke: 0.34vw #595959;
  margin-top: -2.6vw;
}
