    ...
    <div class="mainslider" :style="zindexStyle">
      <div class="slider">
        <div>
          <button
            :class="lefticon"
            @mouseleave="righticon = 'right-icon' , rectangleBoxShow = false"
            @mouseover="righticon = 'right-icon-active' , rectangleBoxShow = true"
            v-if="leftIconShow"
            :disabled="isDisable"
            @click="leftClick
            ">
        ...
        <div>
          <button
            :class="righticon"
            @mouseleave="lefticon = 'left-icon' , rectangleBoxShow = false"
            @mouseover="lefticon = 'left-icon-active' , rectangleBoxShow = true"
            :disabled="isDisable"
            @click="rightClick"
          >
      ...
      <div class="sbox">
        <div class="main">
          <div :style="ClickTransform" class="overflowClass">
            <card
              @rowCount="updateLength($event)"
              :rightClickCounter="rightclickcounter"
              @mouseenter.native="mouseEnterCard"
              @mouseleave.native="mouseLeaveCard"
              :database="database"
              :type="type"
              :filterData="filterData"
              :searchValue="searchValue"
              class="topPosition"></card>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    leftClick() {
      ...
      this.buttonLimit--;
      this.translateX = this.translateX + 93.5;
      this.ClickTransform = `transform: translateX(${ this.translateX }vw);transition: 1s;`;
  ...
  mounted() {
    this.sliderBox = Math.floor(this.length / 6);
  }
};
</script>
